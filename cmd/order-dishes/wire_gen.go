...
func wireApp(serviceName string) (*kratos.App, func(), error) {
	...
	dishes := dao.NewDishes(db, logger)
	dtoDto := dto.NewDto()
	snowflakeSnowflake, err := snowflake.NewSnowflake()
	if err != nil {
		return nil, nil, err
	}
	dishesService := service.NewDishesService(dishes, logger, dtoDto, snowflakeSnowflake)
	grpcServer, err := server.NewGRPCServer(configServer, dishesService)
	if err != nil {
		return nil, nil, err
	}
	httpServer, err := server.NewHTTPServer(configServer, dishesService)
	if err != nil {
		return nil, nil, err
	}
